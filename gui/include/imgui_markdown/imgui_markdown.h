...
namespace ImGui
{
    ...
    struct MarkdownConfig
    {
        ...
        ImVec4                   codeBg = ImVec4( 0.16f, 0.17f, 0.19f, 1.0f );  // (legacy) lighter-than-pane background for inline code spans and fenced code blocks
        ImVec4                   codeText = ImVec4( 0.85f, 0.87f, 0.90f, 1.0f ); // (legacy) text color for code spans
        ImFont*                  codeFont = nullptr;                          // italic font for inline code spans and fenced code blocks; when set, code renders as this font instead of a lighter background box
    };
    ...
    struct Line {
        ...
        bool isCode = false;            // inline code span (single backtick pair)
    ...
    inline void RenderLine( const char* markdown_, Line& line_, TextRegion& textRegion_, const MarkdownConfig& mdConfig_ )
    {
		...
		else if( line_.isCode )             // render inline code span
		{
    ...
    // Render a fenced code block (three-backtick delimited) as italic text,
    // preserving internal newlines. No background box and no syntax highlighting
    // is attempted.
    inline void RenderFencedCodeBlock( const std::string& text, const MarkdownConfig& mdConfig_ )
    {
        if( text.empty() )
        {
            return;
        ...
        if( mdConfig_.codeFont )
        {
            ImGui::PushFont( mdConfig_.codeFont );
        }
    ...
    inline void Markdown( const char* markdown_, size_t markdownLength_, const MarkdownConfig& mdConfig_ )
    {
        ...
        for( int i=0; i < (int)markdownLength_; ++i )
        {
            ...
            // Test to see if we have inline code styling (single backtick pair)
			switch( code.state )
			{
			case InlineCode::NONE:
				if( c == '`' && link.state == Link::NO_LINK && !line.isHeading )
				{
					code.state = InlineCode::OPEN;
					code.text.start = i;
				}
				break;
			case InlineCode::OPEN:
				if( c == '`' )
				{
					code.text.stop = i;
					// Render the text up to the opening backtick.
					if( code.text.start > line.lineStart )
					{
						line.lineEnd = code.text.start;
						RenderLine( markdown_, line, textRegion, mdConfig_ );
						ImGui::SameLine( 0.0f, 0.0f );
						line.isUnorderedListStart = false;
						line.leadSpaceCount = 0;
					}
					// Render the inline code span as CODE.
					line.isCode = true;
					line.lastRenderPosition = code.text.start;
					line.lineStart = code.text.start + 1;
					line.lineEnd = code.text.stop;
					RenderLine( markdown_, line, textRegion, mdConfig_ );
					ImGui::SameLine( 0.0f, 0.0f );
					line.isCode = false;
					line.lastRenderPosition = i;
            ...
            if( c == '\n' )
            {
                // Detect a fenced code block marker: a line whose non-space
                // content is exactly three backticks. It toggles code-block mode;
                // content lines in between are buffered and rendered as one block.
                bool fenceLine = false;
                {
                    int s = line.lineStart;
                    int e = i;
                    while( s < e && ( markdown_[ s ] == ' ' || markdown_[ s ] == '\t' ) )
                    {
                        ++s;
                    }
                    if( ( e - s ) >= 3 && markdown_[ s ] == '`' && markdown_[ s + 1 ] == '`' && markdown_[ s + 2 ] == '`' )
                    {
                        int t = s + 3;
                        while( t < e && markdown_[ t ] == ' ' )
                        {
                            ++t;
                        }
                        fenceLine = ( t == e );
                ...
                else
                {
                    if( fenceLine )
                    {
                        RenderFencedCodeBlock( fencedBlockBuf, mdConfig_ );
                        inFencedBlock = false;
                        fencedBlockBuf.clear();
                    }
                    else
                    {
                        // Buffer the raw content line, preserving its newline.
                        fencedBlockBuf.append( markdown_ + line.lineStart, (size_t)( i - line.lineStart ) );
                        fencedBlockBuf.push_back( '\n' );
                    }
                }
    ...
    inline void defaultMarkdownFormatCallback( const MarkdownFormatInfo& markdownFormatInfo_, bool start_ )
    {
        ...
        case MarkdownFormatType::CODE:
            if( start_ )
            {
                // Inline code span: render in the italic code font so the span reads
                // as set-off text instead of a colored background box.
                if( markdownFormatInfo_.config->codeFont )
                {
                    ImGui::PushFont( markdownFormatInfo_.config->codeFont );
                }
            }
            else
            {
