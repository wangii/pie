...
### 10.1 Materialize terminal Action feedback

When `_applyProductionControl` handles a controller-authored terminal decision, materialize a durable Observation before appending the Action transition:

- `unresolvable_action` → Observation with `statement` = terminal reason, `sourceEventIds` = the episode's finalized execution results, `affects` = `frame`;
- `escalate_action` → Observation with `affects` = the challenged object (`frame` or `anchor`).

Materialize only when the episode produced at least one finalized result to cite; a reason without provenance is not material evidence. Harness-generated terminal transitions (Frame lease expiry, evidence-round exhaustion) remain raw events and do not materialize: they are bounded-return mechanics, not controller feedback. Routine command errors remain raw events. No new persisted field or primitive is introduced; `statement` is the controller's own reason, not a compiler summary.

### 10.2 Make the feedback verb crisp

Extend the epistemic control prompt so the controller distinguishes the two terminal verbs by what the episode revealed:

- use `escalate_action(challenge)` when the finalized results contradict or undermine the Frame relation or Anchor success semantics;
- use `unresolvable_action` when the episode cannot complete under current constraints, naming in `reason` what was found and what remains unknown.

This complements the existing `unresolvable_action` re-adjudication guidance (falsify/kill when the reason contradicts the Frame premise).

### 10.3 Deterministic coverage

Add faux-provider tests proving:

- `unresolvable_action` and `escalate_action` each materialize exactly one durable Observation whose statement is the terminal reason, whose provenance is the episode's finalized results, and whose `affects` target is correct;
...
- `complete_action` materializes no Observation;
...
- controller-authored terminal Action outcomes materialize durable Observations with exact provenance and correct `affects`;
...
- no new primitive, ontology, persisted field, or transcript summary is added.
