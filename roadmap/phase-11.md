...
- Observation statements are controller-provided direct relation assertions; `_validateObservationStatement` rejects a bare confirmation token, an experiment record (`Expectation:`/`Prediction error:`), a process record (`we did not prove X`, `budget exhausted`, `the action completed`), and a search-failure record (`could not be located`, `no file contains`, `remain unidentified`), so only a named world relation crosses into the epistemic layer. A negated fact about a named referent still passes. `expectation` and `predictionErrorSign` remain provenance metadata on the entry.
