...
import type { RpcCommand, RpcDomainSnapshot, RpcResponse, RpcSessionState, RpcSlashCommand } from "./rpc-types.ts";
...
export class RpcClient {
	...
	/**
	 * Get the replayed domain snapshot: the tasks, beliefs, and formulation state of the current
	 * branch. A reconnecting client reads this once and then continues from the event stream.
	 */
	async getDomainSnapshot(): Promise<RpcDomainSnapshot> {
		const response = await this.send({ type: "get_domain_snapshot" });
		return this.getData(response);
	}
