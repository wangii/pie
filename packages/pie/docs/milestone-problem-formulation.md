...
## 后续增量：发布回应与 focus 重审（schema v5）

以下增量补充原 M1–M5 契约，覆盖下文“发布后重新选择”的旧流程：

- 任何一次发布（含首版）都暂停当前运行、等待用户对该版本的回应，任务保持 active；
  不需用户确认的只是尚无 Frame 时的初步调查。
- 等待状态绑定任务与发布版本。普通用户回复进入现有纠正通道；旧版本回应、先前排队
  的输入或扩展生成消息不能代替本版本的回应。没有超时自动批准。
- propose 回应全部待处理输入后，必须显式重审 focus，才允许路由、实验选择或结案。
  同一 belief 集合可以是有效重审，`FocusDeclared.formulation` 记录对应版本。
- 重审关注对象、关系、尺度及其任务相关性，不机械对应文件／组件／系统层级；不改变
  belief 真值、不清除已有观察或未裁定责任，也不限制反例实验。
- 新增 `Task.formulationReview`，通过现有发布、纠正与 focus 事件折叠；恢复同时读回
  beliefs、focus 与版本化门槛。领域协议升级到 v5，明确拒绝旧日志，不迁移或改写。
- 终端显示等待及重审状态；普通回复继续原任务。`/frame correct` 只记录纠正，空闲时
  再提交普通输入启动处理。RPC 快照暴露相同状态；本增量没有修改原生 GUI。
...
补充说明：`interpretation` 表达的是当前理解下对已有 beliefs 的组织方式与关键区分（它们在此
理解下如何相关、哪些区分对本任务重要），不是 belief 状态的摘要，也不要求互斥穷尽的分类——
可重叠，也可暂时不归类。上述决策表不因此改变；运行时仍只校验必填与相同内容重复提交。
