...
This page gets you from a source checkout to a useful first pie session.

## Install

Pie is a private, unpublished package in the pi monorepo; there is no npm
distribution. Run pie from a source checkout:

```bash
git clone <repo> && cd <repo>
npm install --ignore-scripts
./pie.sh          # start pie in the current directory
```

`--ignore-scripts` disables dependency lifecycle scripts during install. Pie
does not require install scripts for normal installs.

All `pi` commands in the rest of this documentation are run through `./pie.sh`
(for example `./pie.sh -p "Summarize this codebase"`).

Pie intentionally shares pi's runtime state: sessions, settings, credentials,
and installed packages live in `~/.pi/agent/` (or the project `.pi/`), and
`PI_*` environment variables are honored. If you switch between pie and the
upstream pi CLI, both read and write the same state.
