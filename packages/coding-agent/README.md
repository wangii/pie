...
- **Footer** - Working directory, session name, total token/cache usage (`↑` input, `↓` output, `R` cache read, `W` cache write, `CH` latest cache hit rate), cost, context usage, current model. In a belief loop the footer shows the epistemic, distillation, and execution roles' models and each role's latest cache hit rate (`epi…% dist…% exec…%`) instead of the single model/CH. Totals include assistant responses, usage reported by tools, and summary generation.
